...
#define ITERATIONS 100000  // Number of loops for benchmarking
...
void report(const char* name, int passed, reg_t sw_res, reg_t hw_res, uint32_t t_sw, uint32_t t_hw) {
    ...
    uint32_t integer_part = ratio/100;
    uint32_t decimal_part = ratio%100;

    if (passed) {
        printf("[PASS] %-6s | Result: 0x%08" PRIreg " | SW: %lu cyc | HW: %lu cyc | Speedup: %lu.%02lux\n", 
               name, hw_res, t_sw, t_hw, integer_part, decimal_part);
    } else {
        printf("[FAIL] %-6s | Expected: 0x%08" PRIreg " | Got: 0x%08" PRIreg " | SW: %lu cyc | HW: %lu cyc\n", 
...
int zbkb_test_benchmark(void) {
    printf("--------------------------------------------");
    printf("\nZbkb Plugin VexRiscv: Verify & Benchmark (N=%d iterations)\n", ITERATIONS);
